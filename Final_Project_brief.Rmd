...
Due to the very similar nature of the methodology for all dependent
variables, we've decided that in order to avoid repetition and respect
the page limit, we will be showcasing the methodology of only one
dependent variable. For any areas where the approach may alter, notes
will be provided to better understand discrepancies, as well as grasping
the forecasting aspect.

The variable we decided to showcase is Real GDP growth, and only the
code and analysis of that variable will be shown for understanding
purposes.
...
The time series of Real GDP and its predictors are shown above. After
transforming the series to show growth from previous lag, we can observe
...
This suggests that no differencing might be required to achieve being
stationary, which is essential for accurate forecasting.
...
most part besides again at the COVID section.
...
from being used in the models; however, the other variables did show
some good correlation at some lags, as we can see. Notably, from
investment showing decent correlation at lag 1, 4 and 9.
...
We specified the test set from 2021 onward so that we could capture the
...
first quarter of 2020 in order to capture the residual errors of the
financial crisis of 2008 as well as from the start of COVID.
...
y_t &= \beta_0 + \beta_{1,1} g_{t-4} + \beta_{1,1} i_{t-2} + \varepsilon_t +dummy_2008Q1 + dummy_2008Q2 + dummy_2008Q3 + dummy_2008Q4  + dummy_2020Q1 + dummy_2020Q2
...
seasonal dummies that are not shown for brevity as well as dummy
variables for the 2008 year and the first two quarters of 2020. Note
that we ended up choosing $y_{t-1}$ and $s_{t-1}$, which are exactly the
...
I used the ARIMA() in order to generate future values for our predictors
based on their historical data and the automatic algorithm from ARIMA()
...
Forecasting next 4 quarters for Real GDP Growth:
...
Forecasting next 12 months for CPI growth:
...
Forecasting next 4 quarters for Real Non-Residential Fixed Investments:
...
Forecasting next 12 months for Average Weekly Earnings:
...
wasn't always the case that the winning model had the lowest RMSE value.
We use AIC and RMSE as a means to compare and pick models, but when
their results seem to oppose, we would tend to pick the model based on
AIC, due to its relative superiority. Perhaps this wasn't the best
...
Forecasting key economic indicators—Real GDP, CPI, Average Weekly
Earnings, and Real Nonresidential Fixed Investment—is crucial for
shaping economic policies and investment decisions. This complex task
hinges on analyzing vast amounts of data and understanding the interplay
between various economic forces. We employed a range of methodologies,
including time series analysis and econometric modeling to predict
future economic conditions. In the end, forecasting remains challenging
due to the unpredictable nature of economic shocks, such as geopolitical
events or pandemics. Economic forecasting is indispensable for
navigating the future economic landscape. It requires a balance of
technical expertise and an understanding of global dynamics, with the
aim of supporting informed decision-making and fostering a stable
